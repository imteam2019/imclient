 ...
 * 加密函数
 * @param key 加密密码
 * @param data  入参为待加密的原文，处理完毕后保存密文。
 * @param blocks    8*"blocks" bytes
 * @return 1:成功, 其他:失败.
 */
 ...
 * 解密函数
 * @param key 解密密码
 * @param data  入参为待解密的密文，处理完毕后保存明文。
 * @param blocks    8*"blocks" bytes
 * @return 1:成功, 其他:失败.
 */
...
/**
 * @brief DES::yencrypt
 * 按指定的方法加密任何大小的数据，在调用之前，复制data到一个新的缓冲区，缓冲区大小调用DES:extend计算而来。
 * @param key
 * @param data
 * @param size
 * @return
...
/**
 * @brief DES::ydecrypt
 * 按指定的方法解密任何大小的密文数据，在调用之前，复制data到一个新的缓冲区，缓冲区大小调用DES:extend计算而来。
 * @param key
 * @param data
 * @param blocks
 * @param size
...
/**
 * @brief DES::usekey
 * 仅被DES::cookey使用,加载内部带数据的key register
 * @param from
 */
