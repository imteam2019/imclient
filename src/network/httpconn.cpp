...
//        this->request->setHeader(QNetworkRequest::ContentTypeHeader,"application/x-www-form-urlencoded");
        this->request->setRawHeader("Accept","application/json,text/html, application/xhtml+xml, */*");
...
//        this->request->setRawHeader("X-Requested-With","XMLHttpRequest");
        this->request->setRawHeader("User-Agent","Mozilla/5.0 (compatible; MSIE 9.0; Windows NT 6.1; WOW64; Trident/5.0)");
 ...
 */
QString HTTPConn::URL(){
    return this->qUrl->toString();
}
...
void HTTPConn::getData(PFUNC pfun, int rePlyFlag,QString strData, void *pUser){
    if(""!=strData){
...
/**
 * @brief HTTPConn::replyFinished 服务器响应结束后的数据处理，按照返回状态调用回调函数
...
void HTTPConn::replyFinished(QNetworkReply *reply)
{
    ...
    QString all;
    int intHttpStatusCode=reply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt();
    QString strHttpStatusMessage=reply->attribute(QNetworkRequest::HttpReasonPhraseAttribute).toString();
    ...
    if(intHttpStatusCode==HTTPConn::HttpResponseStatus::HTTPStatusOK){
       ...
       qDebug()<<all<<endl;
        ...
        this->getData(this->pf,intHttpStatusCode,strHttpStatusMessage,this->pUesr);
...
void HTTPConn::printRequestHeasers(QNetworkRequest * request){
    ...
    qDebug()<<request->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt()<<endl;
...
void HTTPConn::printResponseHeaders(QNetworkReply *reply){
    ...
    qDebug()<<reply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt()<<endl;
