...
CollapseViewItem::CollapseViewItem(QString titleText, QWidget *parent)
    : QWidget(parent), ui(new Ui::CollapseViewItem) {
  ui->setupUi(this);
  ...
  for (int i = 0; i < personNumber; i++) {
    //随机选择好友头像
    QString picPath =
        QString(":/media/person/media/person/%1.jpg")
  ...
  titleText = QString("%1 (%2/%3)")
                  .arg(titleText, QString::number(offlinPerson),
                       QString::number(personNumber));
...
CollapseViewItem::~CollapseViewItem() { delete ui; }
...
void CollapseViewItem::addSubItems(QList<QWidget *> widgets) {
  foreach (QWidget *var, widgets) { addSubItem(var); }
}
