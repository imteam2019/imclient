...
NewFriend::NewFriend(QWidget *parent) : QDialog(parent), ui(new Ui::NewFriend) {
  ui->setupUi(this);
  ui->tetext->setPlainText("123abc中文到底行不行？");
}

NewFriend::~NewFriend() { delete ui; }

void NewFriend::on_btnAdd_clicked() {
  if (ui->leID->text().length() <= 0) {
    QMessageBox::information(
        nullptr, "Error:", "请输入对方的ID（可能是邮箱地址或电话号码等）。");
    return;
  }
...
void NewFriend::on_btdEn_clicked() {
  ...
  std::cout << "解密后Base64解码：" << text << endl;
